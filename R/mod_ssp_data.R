      ...
      bslib::layout_columns(
        ...
        bslib::accordion(
          ...
          bslib::accordion_panel(
            ...
            bslib::layout_columns(
              ...
              shiny::selectInput(
                ...
                choices  = c("0.6" = 0.6, "0.8" = 0.8, "1.0" = 1.0, "1.2" = 1.2, "1.4" = 1.4),
                selected = 0.6
              ),
            ...
            bslib::layout_columns(
              ...
              shiny::selectInput(
                ...
                label    = name_with_info("Mean (log scale)", "Mean of item RT residual variance. Lower = less residual noise in RT."),
                choices  = c(
                  "log(0.2)" = log(0.2),
                  "log(0.4)" = log(0.4),
                  "log(0.6)" = log(0.6),
              ...
              shiny::tags$fieldset(
                ...
                shiny::numericInput(ns("sdlog_sigma2"),
                                    label = name_with_info("SD (log scale)", "Standard deviation of item RT residual variance. Lower = less variation noise in RT."),
          ...
          bslib::accordion_panel(
            ...
            shiny::helpText(
              ...
              "Set 'none' to exclude a parameter."
            ),
            ...
            bslib::layout_columns(
              ...
              shiny::selectInput(
                ...
                choices  = c("none" = "NULL", "0.20" = 0.2, "0.15" = 0.15, "0.10" = 0.1, "0.05" = 0.05),
                selected = 0.1
              ),
        ...
        bslib::accordion(
          ...
          bslib::accordion_panel(
            ...
            # Top row: sample size results
            shiny::uiOutput(ns("out_header")),

            # Item & Person parameter tables
            ...
            shiny::div(
              ...
              bslib::navset_pill(
                    ...
                    bslib::card_body(
                      bslib::layout_columns(
                        ...
                        shiny::selectInput(
                          ...
                          label = name_with_info("Parameters", "Show metrics for estimated item or person parameters."),
                ...
                bslib::nav_panel(
                  ...
                  bslib::card(
                    bslib::card_header("R\u0302 Convergence Diagnostics", class = "bg-light"),
                    bslib::card_body(
                      shiny::helpText(
                        "Quantiles of the R\u0302 statistic across Monte Carlo iterations."
                      ),
                      shiny::uiOutput(ns("convergence_table"))
                    )
                  )
                )
              )
...
mod_ssp_data_server <- function(id) {
  ...
  moduleServer(id, function(input, output, session) {
    ...
    ssp_result <- shiny::reactive({
      ...
      if (!any(active)) {
        return(list(
          object = NULL,
          design = NULL,
          error = "At least one RMSE threshold must be set."
        ))
      }
      ...
      shiny::tagList(
        ...
        bslib::layout_columns(
          ...
          metric_card(
            ...
            value      = paste0(result$design$out.par, " \u2264 ", result$design$thresh),
            tooltip    = "The item parameter and RMSE threshold that required the largest sample size, thus determining the minimum N.",
            icon_name  = "funnel",
    ...
    # Convergence table (Diagnostics tab)
    output$convergence_table <- shiny::renderUI({
      ...
      rhat_dat <- obj$comp.rmse$rhat.dat
      if (is.null(rhat_dat)) {
        return(shiny::tags$div(
          class = "text-muted text-center py-4",
          "No R\u0302 data available."
        ))
      }

      if (is.matrix(rhat_dat)) rhat_dat <- as.data.frame(rhat_dat)
      ...
      # Build header
      header_cells <- list(
        shiny::tags$th(style = "text-align: left; padding: 6px 10px;", "Quantile")
      )
      for (col in colnames(rhat_dat)) {
        display <- display_names[[col]] %||% col
        header_cells <- c(header_cells, list(
          shiny::tags$th(style = "text-align: right; padding: 6px 10px;", display)
        ))
      }

      # Build rows
      rows <- lapply(rownames(rhat_dat), function(rn) {
        cells <- list(
          shiny::tags$td(
            style = "text-align: left; padding: 6px 10px; font-weight: 600;",
            rn
        ...
        for (col in colnames(rhat_dat)) {
          val <- rhat_dat[rn, col]
          # Color-code: green if < 1.05, amber if < 1.10, red otherwise
          color <- if (val < 1.05) "#198754" else if (val < 1.10) "#fd7e14" else "#dc3545"
          cells <- c(cells, list(
            shiny::tags$td(
              style = paste0(
                "text-align: right; padding: 6px 10px; font-family: monospace; color: ", color, ";"
              ),
              sprintf("%.6f", val)
            )
          ))
